  ...
  if (magicJS.isResponse) {
    switch (true) {
      ...
      case /^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test(magicJS.request.url):
        try {
          // 尝试使用tab name直观修改
          const tabNameList = new Set(["推薦"]);
          // 107 概念版游戏中心，获取修改为Story模式
          const topList = new Set([176, 222, 107]);
          // 102 开始为概念版id
          const bottomList = new Set([177, 179, 181, 102, 104, 106]);
          let obj = JSON.parse(magicJS.response.body);
          if (obj["data"]["tab"]) {
          ...
          if (obj["data"]["top"]) {
            let top = obj["data"]["top"].filter((e) => {
              if (e.id === 222 || e.id === 107) {
                e.uri = "bilibili://link/im_home";
                e.icon = "http://i0.hdslb.com/bfs/archive/d43047538e72c9ed8fd8e4e34415fbe3a4f632cb.png";
                e.tab_id = "Story_Top";
                e.name = "Story";
              }
